...
struct Year2020Day2: DayBase {
  func part1(_ input: String) -> CustomDebugStringConvertible {
    input.policyAndPasswords.filter(\.isValidCount).count
  }

  func part2(_ input: String) -> CustomDebugStringConvertible {
    input.policyAndPasswords.filter(\.isValidPosition).count
  }
}
    ...
    lines.compactMap { line in
      ...
      return .init(
        policy: .init(
          minCount: minCount,
          maxCount: maxCount,
          letter: String(line[letterRange])
        ),
        password: String(line[passwordRange])
      )
    }
  }
}

extension Year2020Day2 {
  struct Policy {
    let minCount: Int
    let maxCount: Int
    let letter: String
  ...
  struct PolicyAndPassword {
    ...
    var isValidCount: Bool {
      let letterCount = self.password.filter { String($0) == self.policy.letter }.count
      return (self.policy.minCount ... self.policy.maxCount).contains(letterCount)
    }

    var isValidPosition: Bool {
      let firstIndex = self.password.index(self.password.startIndex, offsetBy: self.policy.minCount - 1)
      let secondIndex = self.password.index(self.password.startIndex, offsetBy: self.policy.maxCount - 1)
      let firstPositionMatches = String(password[firstIndex]) == self.policy.letter
      let secondPositionMatches = String(password[secondIndex]) == self.policy.letter
