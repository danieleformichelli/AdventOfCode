...
**/
struct Day12: DayBase {
  func part1(_ input: String) -> Any {
    var moonPositionsPerCoordinate = self.moonPositions
    var moonVelocitiesPerCoordinate = self.moonPositions.map { $0.map { _ in 0 } }
    ...
    let coordsCount = 3
    return (0..<moonsCount).map { moonIndex in
      let potentialEnergy = (0..<coordsCount).map { abs(moonPositionsPerCoordinate[$0][moonIndex]) }.reduce(0, +)
      let kineticEnergy = (0..<coordsCount).map { abs(moonVelocitiesPerCoordinate[$0][moonIndex]) }.reduce(0, +)
...
extension Day12 {
  var moonPositions: [[Int]] {
    ...
    self.inputLines.forEach { line in
