...
object Day22 {
    ...
    */
    fun part1(caveScan: List<String>): Int {
        val (depth, target) = parseInput(caveScan)

        val erosionLevels = HashMap<Coordinate, Int>()
        val regionTypes = HashMap<Coordinate, RegionType>()
        var totalRiskLevel = 0
        for (y in 0..target.y) {
            for (x in 0..target.x) {
                val coordinate = Coordinate(x, y)
                computeRegionErosionLevelAndType(coordinate, target, depth, erosionLevels, regionTypes)
                totalRiskLevel += regionTypes[coordinate]!!.risk()
            }
        }

        return totalRiskLevel - regionTypes[Coordinate(0, 0)]!!.risk() - regionTypes[target]!!.risk()
    }
    ...
    */
    fun part2(caveScan: List<String>): Int {
        var currentBest = Int.MAX_VALUE
        val (depth, target) = parseInput(caveScan)

        val erosionLevels = HashMap<Coordinate, Int>()
        val regionTypes = HashMap<Coordinate, RegionType>()
        val minMinutes = HashMap<Pair<Coordinate, Tool>, Int>()
        val queue = PriorityQueue<IntermediateResult>(compareBy { it.minutes })
        queue.add(IntermediateResult(Coordinate(0, 0), 0, Tool.TORCH))
        while (!queue.isEmpty()) {
            val current = queue.poll()

            if (current.minutes >= currentBest) {
                // cost is already higher than best found solution
                continue
            }

            val coordinateAndTool = Pair(current.coordinate, current.tool)
            if (minMinutes[coordinateAndTool] ?: Int.MAX_VALUE <= current.minutes) {
                // cost is already higher than another instance
                continue
            } else {
                minMinutes[coordinateAndTool] = current.minutes
            }

            if (current.coordinate == target) {
                currentBest = Math.min(currentBest, if (current.tool == Tool.TORCH) current.minutes else current.minutes + ChangeItemTime)
                continue
            }

            computeRegionErosionLevelAndType(current.coordinate, target, depth, erosionLevels, regionTypes)
            for (tool in Tool.values()) {
                if (tool != current.tool && tool.canBeUsedIn(regionTypes[current.coordinate]!!)) {
                    queue.add(IntermediateResult(current.coordinate, current.minutes + ChangeItemTime, tool))
                }
            }

            for (nextCoordinate in current.coordinate.adjacentsWihoutDiagonals()) {
                if (nextCoordinate.x < 0 || nextCoordinate.y < 0) {
                    // position is out of region
                    continue
                }

                computeRegionErosionLevelAndType(nextCoordinate, target, depth, erosionLevels, regionTypes)
                if (current.tool.canBeUsedIn(regionTypes[nextCoordinate]!!)) {
                    queue.add(IntermediateResult(nextCoordinate, current.minutes + ChangeRegionTime, current.tool))
                }
            }
        }
    ...
    data class IntermediateResult(val coordinate: Coordinate, val minutes: Int, val tool: Tool)

    fun parseInput(caveScan: List<String>): Pair<Int, Coordinate> {
        val depth = caveScan[0].removePrefix("depth: ").toInt()
        val coordinateStr = caveScan[1].removePrefix("target: ").split(",")
        return Pair(depth, Coordinate(coordinateStr[0].toInt(), coordinateStr[1].toInt()))
    }

    enum class Tool {
        CLIMBING_GEAR, TORCH, NOTHING;

        fun canBeUsedIn(regionType: RegionType) = when (this) {
            CLIMBING_GEAR -> regionType != RegionType.NARROW
            TORCH -> regionType != RegionType.WET
            NOTHING -> regionType != RegionType.ROCKY
        }
    }
    ...
    enum class RegionType {
        ...
        companion object {
            fun fromErosionLevel(erosionLevel: Int) = when (erosionLevel % 3) {
                0 -> ROCKY
                1 -> WET
                2 -> NARROW
                else -> throw Exception("This can never happen")
            }
        }
    }

    private fun computeRegionErosionLevelAndType(region: Coordinate, target: Coordinate, depth: Int, erosionLevels: HashMap<Coordinate, Int>, regionTypes: HashMap<Coordinate, RegionType>) {
        if (erosionLevels[region] != null && regionTypes[region] != null) {
            return
        }
        ...
        val geologicIndex = when {
            ...
            region == target -> 0
                ...
                val previousRow = Coordinate(region.x, region.y - 1)
                val previousCol = Coordinate(region.x - 1, region.y)
                computeRegionErosionLevelAndType(previousRow, target, depth, erosionLevels, regionTypes)
                computeRegionErosionLevelAndType(previousCol, target, depth, erosionLevels, regionTypes)
                erosionLevels[previousRow]!! * erosionLevels[previousCol]!!
            }
        }

        val erosionLevel = (geologicIndex + depth) % 20183
        erosionLevels[region] = erosionLevel

        val regionType = RegionType.fromErosionLevel(erosionLevel)
        regionTypes[region] = regionType
    }

    private const val ChangeItemTime = 7
    private const val ChangeRegionTime = 1
}