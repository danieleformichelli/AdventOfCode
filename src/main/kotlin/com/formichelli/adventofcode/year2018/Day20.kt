...
object Day20 {
    ...
    */
    fun part1(instructionsRegex: String): Int {
        return part1Helper(instructionsRegex).maxBy { it.value }!!.value
    }
    ...
    */
    fun part2(instructionsRegex: String): Int {
        return part1Helper(instructionsRegex).count { it.value >= 1000 }
    }

    private fun part1Helper(instructionsRegex: String): Map<Coordinate, Int> {
        val distancesMap = HashMap<Coordinate, Int>()
        var currentCoordinate = Coordinate(0, 0)
        distancesMap[currentCoordinate] = 0
        val forks = Stack<Coordinate>()
        for (c in instructionsRegex) {
            when (c) {
                '^', '$' -> {
                    // nothing to do
                }
                '(' -> {
                    // start of branch or path
                    forks.push(currentCoordinate)
                }
                ')' -> {
                    // end of branch or path
                    currentCoordinate = forks.pop()
                }
                '|' -> {
                    currentCoordinate = forks.peek()
                }
                else -> {
                    val newCoordinate = newCoordinate(currentCoordinate, c)
                    distancesMap[newCoordinate] = Math.min(distancesMap[newCoordinate]
                            ?: Int.MAX_VALUE, distancesMap[currentCoordinate]!! + 1)
    ...
    private fun newCoordinate(currentCoordinate: Coordinate, direction: Char): Coordinate {
        when (direction) {
            'N' -> {
