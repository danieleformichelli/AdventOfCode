...
object Day21 {
    ...
    */
    fun part1(opCodes: List<String>): Int {
        val (ipRegister, opCodesAndInputs) = Day19.parseInput(opCodes)
        ...
        while (ip < opCodesAndInputs.size) {
            ...
            if (ip == 28) {
                // code terminates when reaching ip 28 and registers[5] == registers[0]
                break
            }
        }
    ...
    */
    fun part2(opCodes: List<String>): Int {
        ...
        val registers = IntArray(6)
        var ip = registers[ipRegister]
        val exitValues = HashSet<Int>()
        var lastExitValues = 0
        while (ip < opCodesAndInputs.size) {
            opCodesAndInputs[ip].execute(registers)
            ...
            if (ip == 28) {
                // last value before looping is the one which halts the execution later
                val exitValue = registers[5]
                if (!exitValues.add(exitValue)) {
                    break
                }
                lastExitValues = registers[5]
            }
        }

        return lastExitValues
    }
}