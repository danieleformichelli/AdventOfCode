package com.formichelli.adventofcode.year_2017.day10
...
fun main(args: Array<String>) {
    ...
    input.forEach { length ->
        ...
        index = (index + length + skipSize++) % list.size
    }

    println(list[0] * list[1])
}

fun reverse(list: IntArray, from: Int, length: Int) {
    for (i in 0..((length - 1) / 2)) {
        val fromIndex = (from + i) % list.size
        val toIndex = (from + length - i - 1) % list.size
